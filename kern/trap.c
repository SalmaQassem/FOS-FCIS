...
void page_fault_handler(struct Env * curenv, uint32 fault_va)
{
	...
	if(env_page_ws_get_size(curenv) < curenv->page_WS_max_size){
		...
		if(ret == E_PAGE_NOT_EXIST_IN_PF){
			if(fault_va >= USER_HEAP_MAX && fault_va < USTACKTOP){
				...
				ptr_frame_info = get_frame_info(curenv->env_page_directory, (void*)fault_va, &ptr);
	...
	else
	{
		if(isPageReplacmentAlgorithmModifiedCLOCK()){
			...
			int found = 0;
	        Try1:;
	        int start = curenv->page_last_WS_index;
			for(int i=0; i<curenv->page_WS_max_size; i++){
				uint32 va = env_page_ws_get_virtual_address(curenv, start);
				...
				start += 1;
				if(start >= curenv->page_WS_max_size){
					start = 0;
				}
			}
			if(found == 0){
				start = curenv->page_last_WS_index;
				goto Try2;
			}
			else{
				found = 0;
			...
			exit:;
			uint32 va = env_page_ws_get_virtual_address(curenv, index);
			uint32 page_permissions = pt_get_page_permissions(curenv, va);
			struct Frame_Info* ptr_frame_info;
			uint32* ptr = NULL;
			ptr_frame_info = get_frame_info(curenv->env_page_directory, (void*)va, &ptr);
			...
			if(ret == E_PAGE_NOT_EXIST_IN_PF){
				if(fault_va >= USER_HEAP_MAX && fault_va < USTACKTOP){
					...
					frame_info = get_frame_info(curenv->env_page_directory, (void*)fault_va, &ptr);
