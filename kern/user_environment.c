...
void env_free(struct Env *e)
{
	...
	struct Frame_Info *page_frame_info = NULL;
	int size;
	int index;
	for(int i = 0; i< e->page_WS_max_size; i++){
		uint32 va = env_page_ws_get_virtual_address(e, i);
		page_frame_info = get_frame_info(e->env_page_directory, (void*)va, &ptr_page_table); //page frame info
		int PT_entry = ptr_page_table[PTX(va)];
		if((PT_entry & PERM_PRESENT) == PERM_PRESENT){
			//free page frame
		    page_frame_info->references = 0;
			free_frame(page_frame_info);
			//unmap_frame(e->env_page_directory, (void*)va);
		}
	}
    //free page tables
	struct Frame_Info *page_table_frame_info = NULL;
	int empty = 1;
	for(int i = 0; i< e->page_WS_max_size; i++){
		uint32 va = env_page_ws_get_virtual_address(e, i);
		page_frame_info = get_frame_info(e->env_page_directory, (void*)va, &ptr_page_table); //page frame info
    ...
    //free working set array
	struct Frame_Info *ws_frame_info = NULL;
	uint32 ws_pa = kheap_physical_address((unsigned int) e->ptr_pageWorkingSet);
	ws_pa = ROUNDDOWN(ws_pa, PAGE_SIZE);
	uint32 ws_va = kheap_virtual_address((unsigned int) ws_pa);
	kfree((void*)ws_va);

	struct Frame_Info *page_dir_frame_info = NULL;
	// free page directory
	uint32 dir_addr = ROUNDDOWN(e->env_cr3, PAGE_SIZE);
	uint32 dir_va = kheap_virtual_address((unsigned int) dir_addr);
	kfree((void*)dir_va);*/
