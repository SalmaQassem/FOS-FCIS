...
struct environments running_env[1000];
int p = 0;
...
struct Env* fos_scheduler_MLFQ()
{
	...
	int running_index;
	if(curenv != NULL){
		int length = sizeof(running_env)/sizeof(running_env[0]);
		for(int i = 0; i<length; i++){
			if(curenv->env_id == running_env[i].id){
				running_index = i;
				queue_num = running_env[i].position;
				f = 1;
				break;
			}
		}
		if(f == 1){
			if(queue_num < num_of_ready_queues - 1){
				enqueue(&(env_ready_queues[queue_num + 1]), curenv);
				running_env[running_index].position = queue_num + 1;
			}
			else if(queue_num == num_of_ready_queues - 1){
				enqueue(&(env_ready_queues[queue_num]), curenv);
			}
		}
	}
	...
	for(int i = 0; i<num_of_ready_queues; i++){
		...
		if(size != 0){
			...
			for(int j = 0; j<length; j++){
				if(next_env->env_id == running_env[j].id){
					r = 1;
				}
			}
			if(r == 0){
				running_env[p].id = next_env->env_id;
				running_env[p].position = i;
				p += 1;
			}
