...
void * create_page_table(uint32 *ptr_page_directory, const uint32 virtual_address)
{
	...
	void* PT_addr = (void*)kmalloc(PAGE_SIZE);
	uint32 PT_Va = (uint32)PT_addr;
	uint32* ptr = PT_addr;
	...
	uint32 PT_pa = kheap_physical_address(PT_Va);
	ptr_frame_info = to_frame_info(PT_pa);
	int PT_Dir_indx = PDX(virtual_address);
	ptr_page_directory[PT_Dir_indx] = CONSTRUCT_ENTRY(PT_pa, PERM_USER |PERM_PRESENT| PERM_WRITEABLE);
	tlbflush();
	return PT_addr;
...
void freeMem(struct Env* e, uint32 virtual_address, uint32 size)
{
	...
	uint32 virt_add = virtual_address;
	for(int i=0; i<size; i++){
		pf_remove_env_page(e,virt_add);
		virt_add += PAGE_SIZE;
	}

	struct Frame_Info* ptr_frame_info = NULL;
	uint32* ptr_page_table = NULL;
	virt_add = virtual_address;
	uint32 new_addr = virtual_address;
	for(int i = 0; i < e->page_WS_max_size; i++) {
		uint32 ws_addr = env_page_ws_get_virtual_address(e,i);
		uint32 end = virt_add + (size*PAGE_SIZE);
		if((ws_addr >= virt_add)&&(ws_addr < end)){
			//ptr_frame_info =get_frame_info(e->env_page_directory,(void*)ws_addr,&ptr_page_table);
			//if(ptr_frame_info != NULL){
				//free_frame(ptr_frame_info) ;
			unmap_frame(e->env_page_directory,(void*)ws_addr);
	...
	for(int i=0; i<size; i++) {
		int empty = 1;
		ptr_page_table = NULL;
		get_page_table(e->env_page_directory,(void*)virt_add,&ptr_page_table);
		if(ptr_page_table !=NULL){
			...
			if(empty == 1){
				...
				pd_clear_page_dir_entry(e,(uint32)virt_add);
				tlbflush();
			}
		}
		virt_add+=PAGE_SIZE;
